...
public class QueenScript : MonoBehaviour
{
    ...
    public bool queenMoved;
    private bool queenMovedOnObstacle;
    ...
    private Vector3 startQueenPosition;
    ...
    void Start()
    {
        ...
        startQueenPosition = transform.position;
    }
    ...
    {
        queenMoved = false;
        queenMovedOnObstacle = false;

        // Detecting where is the queen and allowin the queen to be moved only after the green square is displayed and queen is dropped
        if (detectionCan == true && coinScript.isKing == false)
        {
            queenMouseDetection();
        }
        ...
        if (coinScript.isKing == false)
        {
            count += 1;
            if (count == 1)
            {
                diceScript.greenSquarePosition = startQueenPosition;
            }
        }
    ...
    private void queenMouseDetection()
    {
        ...
        if (isDragging && Input.GetMouseButtonUp(0))
        {
            ...
            if (queenMovedOnObstacle == true)
            {
                rb2D.isKinematic = false;
                rb2D.MovePosition(startQueenPosition);
                ...
                foreach (BoxCollider2D collider in colliders)
                {
                    ...
                    if (collider != null)
                    {
                        ...
                        if (GetComponent<BoxCollider2D>().IsTouching(collider))
                        {
                            ...
                            // Additional checking so that the queen is destroyed if the obstacle is encountered
